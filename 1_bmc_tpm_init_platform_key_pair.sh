...
tpm2_createprimary -G ecc -c primary.ctx
# create and output an rsa keypair (rsakey.pub, rsakey.priv) which is protected by the primary key
tpm2_create -G rsa2048 -a "fixedtpm|fixedparent|sensitivedataorigin|userwithauth|decrypt|sign|noda" -C primary.ctx -u network_auth.pub -r network_auth.priv
# remove all transient objects
tpm2_flushcontext -t
# load the rsa keypair into tpm
tpm2_load -C primary.ctx -u network_auth.pub -r network_auth.priv -c platform.ctx
